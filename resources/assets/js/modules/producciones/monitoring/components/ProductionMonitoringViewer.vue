					...
					<div class="col s12 l6">
						<div class="card-panel pm-panel">
							...
							<ul class="browser-default pm-list" v-if="hasProduction">
								...
								<li><strong>Primera fecha consultada:</strong> {{ rFecha.minFecha }}</li>
								<li><strong>Días máx. monitoreo:</strong> {{ fechasCount }}</li>
								<li><strong>Última fecha escena:</strong> {{ rFecha.maxFecha }}</li>
							</ul>
...
<script>
	...
	props: {
		...
		computed: {
			rFecha(){
				if(this.fechasDisponibles.length==0) return{maxFecha:'-',minFecha:'-'};
				const timestamps = this.fechasDisponibles.map(f => new Date(f).getTime());
				const maxFecha = new Date(Math.max(...timestamps));
				const minFecha = new Date(Math.min(...timestamps));
				console.log("Máxima:", maxFecha.toISOString().split('T')[0]);
				console.log("Mínima:", minFecha.toISOString().split('T')[0]);
				return {maxFecha,minFecha}
			},
			fechasCount(){
				return (this.fechasDisponibles??[]).length;
			},
			hasProduction() {
				return !!this.production && Object.keys(this.production).length > 0 && (this.produccion?.folio?? false);
			},
			hasDetail() {
